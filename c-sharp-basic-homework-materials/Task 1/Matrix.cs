...
namespace Task_1
{
    class Matrix
    {
        ...
        public void Move()//Метод отображения одной цепочки
        {
            ...
            while (true)
            {
                count = rand.Next(3, 14);//Метод возвращает случайное значение в указаном промежутке
                lenght = 0;
                Thread.Sleep(100); //Останавливаем поток на время полученое выполнением метода rand.Next в указаном прромеж
